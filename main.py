...
def test(M, LV, GV, N, ALPHA, GAMMA, EPSILON, METHOD, STACT, EPOCHS, OUTPUT_DIR):
...
def main():
    ...
    GAMMA = 0.7     # learning rate (0<γ≤1): the extent to which Q-values are updated every timestep / epoch
    ...
    EPOCHS = 2000       # set number of epochs to train RL model
    OUTPUT_DIR = '../output/'
    ...
    for GAMMA in [x/10 for x in range(1,11)]:
        for ALPHA in [x/10 for x in range(0,11)]:
            for EPSILON in [x/10 for x in range(1,11)]:
                test(M, LV, GV, N, ALPHA, GAMMA, EPSILON, METHOD, STACT, EPOCHS, OUTPUT_DIR)
